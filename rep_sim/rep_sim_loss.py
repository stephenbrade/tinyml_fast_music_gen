...
def rep_similarity_loss(train_model, target_model, rep_sim, inputs, device, student_model = 'ResNet-50', lengths = None, use_noise = False, 
    ...
    if rep_sim == 'CKA':
        ...
        sims = torch.stack(list(layermap_sim(train_model, target_model, student_model, rep_sim, inputs, target_batch, device, lengths = lengths).values()))
        sim = torch.sum(sims)
    elif rep_sim == 'Procrustes':
        sims = torch.stack(list(layermap_sim(train_model, target_model, rep_sim, inputs, target_batch, device, lengths = lengths).values()))
        sim = torch.sum(sims)
    elif rep_sim == 'Ridge':
        sims = torch.stack(list(layermap_sim(train_model, target_model, student_model, rep_sim, inputs, target_batch, device, lengths = lengths).values()))
        sim = torch.sum(sims)
